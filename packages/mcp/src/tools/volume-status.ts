...
export const toolName = 'trench_volume_status';
export const toolDescription =
  'Check the status of the current volume boosting session. Shows trades executed, success rate, volume generated, and uptime.';
...
export async function handler(_args: ToolInput, _config: MCPConfig) {
  ...
  if (sessions.length === 0) {
    return {
      content: [{
        type: 'text' as const,
        text: 'No volume sessions found. Use trench_volume_start to begin.',
      }],
    };
  }
  ...
  for (const session of sessions) {
    ...
    lines.push(`Volume Session: ${session.id}`);
