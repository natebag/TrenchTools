...
TrenchSniper OS is an open-source, self-hosted trading dashboard for Solana token operations. It provides a unified interface for sniping, volume boosting, automated bot trading, wallet management, and portfolio analytics.
...
TrenchSniper OS puts all of these tools in a single self-hosted dashboard:

| Tool | What It Does |
|------|-------------|
| **Sniper** | Buy tokens instantly with auto-sell triggers |
| **Volume Booster** | Multi-wallet volume with configurable patterns |
...
### Auto-Routing
Paste any Solana token CA and TrenchSniper figures out the rest. If the token is on PumpFun's bonding curve, it routes through PumpPortal API. If it's graduated to Raydium/Jupiter, it uses the Jupiter aggregator. You don't need to know which DEX a token is on.

### Real Trades, Not Simulations
Every action in TrenchSniper executes real on-chain transactions (on whichever network you select). Volume boosting creates real swaps. Bot groups generate real trades. P&L is calculated from real transaction history.
